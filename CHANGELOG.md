...
### 修复：TB 油猴脚本「缺陷分类」取名取错 + 长下拉点不到（v2.8.5）`需重装油猴脚本` `已部署 151+127`
- 现象：v2.8.4 日志报 `未找到选项 缺陷分类 一维码/二维码识别 attempt 0/1`。分组名取成了缺陷标题里的模块名，且下拉浮层明显有搜索框和滚动条。
- 原因一（取名取错）：`readSectionNameFromDom()` 用 `a[href*="/bug/section/<id>"]` 子串匹配，而任务行链接是 `/bug/section/<id>/task/<tid>`，同样命中 —— `querySelector` 拿到的是任务行，textContent 就成了任务标题。
- 原因二（点不到）：「缺陷分类」有几十个选项，TB 用搜索框 + 滚动/虚拟化渲染，未渲染的选项在 DOM 里根本查不到，`findOption()` 只看得见头几条。
- 变更：① href 必须**正好以分组 id 结尾**才算侧边栏那条，并去掉文本末尾的条数角标；② `harvestSectionName()` 排除任务型对象（带 `content`/`_tasklistId`/`dueDate`）；③ `selectDropdownOption()` 打开浮层后先把目标值打进搜索框过滤再点（无搜索框的字段自动跳过），严格相等找不到时退化为「互相包含」匹配并取文本最短项；④ 目标值日志挪到选择动作旁边（`缺陷分类目标值（当前分组名）= … | sectionId = …`）便于定位。
- 涉及文件：`backend/scripts/tcmp-tb-filler.user.js`。
- 部署动作：`python deploy/windows/deploy_userscript.py`。
- 验证：两台 `HTTP=200` + `@version 2.8.5`。使用者侧：日志中目标值应等于侧边栏分组名（如"不分类缺陷"），随后 `✓ 已选 缺陷分类 = <分组名>`。
